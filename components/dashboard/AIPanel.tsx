  ...
  return (
    <div className="flex justify-start mb-3">
      <div className="flex gap-2 max-w-[95%]">
        <div className="w-6 h-6 rounded-full bg-gradient-to-br from-violet-600 to-blue-600 flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0 mt-1">
          ✦
        </div>
...
export function AIPanel() {
  ...
  async function runQuery(query: string) {
    ...
    const avgLit = (
      sample.reduce((a, b) => a + b.literacyRate, 0) / sample.length
    ).toFixed(1);

    const avgGdp = (
      sample.reduce((a, b) => a + b.gdpContribution, 0) / sample.length
    ).toFixed(2);

    const avgHealth = (
      sample.reduce((a, b) => a + b.healthIndex, 0) / sample.length
    ).toFixed(1);

    const context = `Total rows: ${
      filteredData.length
    }. States: ${states}. Avg Literacy: ${avgLit}%. Avg GDP: ${avgGdp}%. Avg Health Index: ${avgHealth}.`;
    ...
    await streamGeminiResponse(prompt, (chunk: string) => {
      accumulated += chunk;
    ...
    >
      ...
      <div className="p-4 border-b border-white/5">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-violet-600 to-blue-600 flex items-center justify-center text-[10px] text-white font-bold">
            ✦
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {isAIThinking && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex items-center gap-2 px-4 py-3 bg-white/3 rounded-xl"
          >
      ...
      <div className="p-4 border-t border-white/5">
        <div className="flex gap-2">
          <input
            ...
            onKeyDown={(e) =>
              e.key === "Enter" && !e.shiftKey && runQuery(input)
            }
            placeholder="Ask about the data..."
            className="flex-1 bg-white/4 border border-white/8 rounded-lg px-3 py-2 text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:border-violet-500/50"
          />

          <button
            onClick={() => runQuery(input)}
            disabled={!input.trim() || isAIThinking}
            className="px-3 py-2 bg-gradient-to-r from-violet-600 to-blue-600 rounded-lg text-white text-sm font-medium disabled:opacity-40"
          >
            ↑
          </button>
        </div>
